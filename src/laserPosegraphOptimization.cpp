...
{
    // parse pointclouds
    int historyKeyframeSearchNum = 250; // enough. ex. [-25, 25] covers submap length of 50x1 = 50m if every kf gap is 1m
...
int main(int argc, char **argv)
{
  ...
  // save directories 
  nh.param<std::string>("save_directory", save_directory, "/home/sam/Desktop/test/"); // pose assignment every k m move 
  ...
  // system params 
  nh.param<double>("keyframe_meter_gap", keyframeMeterGap, 0.1); // pose assignment every k m move 
  nh.param<double>("keyframe_deg_gap", keyframeDegGap, 1); // pose assignment every k deg rot 
  keyframeRadGap = deg2rad(keyframeDegGap);

  nh.param<double>("sc_dist_thres", scDistThres, 0.1);  
  nh.param<double>("sc_max_radius", scMaximumRadius, 40.0); // 80 is recommended for outdoor, and lower (ex, 20, 40) values are recommended for indoor 
